...
- **ReAct 代码看护**: Stage 3 代码生成后 ReAct 引擎自动验证（原生 Function Calling 驱动，非文本解析）。验证工具链：`check_imports`（语法 + 导入检测）、`run_module`（子进程运行时验证）、`analyze_error`（错误定位分析）。配置变更上限（%）后自动拦截超大改动，要求 LLM 精简

### 自动化流水线（7 阶段）

| 阶段 | 说明 |
|------|------|
| 1. 设计优化 | LLM 根据用户需求优化 UML 设计，结果推送至 Diff 面板 |
| 2. 开发确认 | 人工审查优化结果，接受/拒绝/追加优化 |
| 3. 代码生成 + 验证 | 从 UML 生成代码或适配已有代码，ReAct 引擎执行代码质量门禁：语法检查（ast.parse）→ 导入检查（子进程 import）→ 运行时验证（python -c），流式逐轮反馈修正 |
| 4. 用例检视 | 加载 Excel 用例库，标记变更 |
| 5. 测试生成 | 增量生成/更新测试代码 |
| 6. 用例调试 | 真实 pytest 执行，修复编译错误（测试侧） |
| 7. 代码优化 | 基于失败反馈优化源码（最多 3 轮，轮间记忆 + 回归回退 + 僵化退出） |

**流水线配置项**: 选择已有源码/测试目录、设置代码变更上限（%），超过阈值自动要求 LLM 精简改动。每次运行自动生成详细 Markdown 报告至 `pipeline_log/` 目录。
...
4. 点击"启动流水线"，按提示操作
5. Stage 3 可展开推理过程查看 ReAct 引擎逐轮验证详情
